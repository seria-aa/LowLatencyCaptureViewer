...
| 화면 표시 방식 | **저지연(최소 지연)**. **VSync(찢어짐 완화)**는 표시 주기를 기다립니다. **호환성 출력(Blt · VSync)**은 다른 출력 경로이며 지연·GPU 부하가 늘 수 있고 HDR10은 지원하지 않습니다 |
...
| PCM 버퍼 목표 | **25ms** |
| 클록 드리프트 보정 | **자동** |
| 콘솔 LPCM 5.1 | **끔(기본)**. 다채널 캡처·출력 장비가 있을 때만 켜는 실험 기능이며 WASAPI Shared 전용 |
...
5.1을 사용하려면 콘솔을 **5.1 LPCM**으로 설정하고, 캡처보드가 PC에 다채널
PCM을 제공해야 합니다. Dolby/DTS 비트스트림은 지원하지 않습니다.
설정 방법과 지원 조건은 [5.1 오디오 안내](docs/AUDIO.ko.md)를 확인하세요.

소리가 간헐적으로 끊기면 Tab 진단창에서 **버퍼 부족** 또는 **리샘플러 출력 부족**이
반복되는지 확인한 뒤 PCM 버퍼를 현재 값에서 5ms씩(예: `20 → 25 → 30ms`) 올리세요. 오류가 없으면 굳이 더
낮추거나 높일 필요가 없습니다.
...
| 원인 확인이 필요함 | 단축키·진단 탭에서 로그 저장을 켠 뒤 문제를 재현하고, **로그 폴더 열기**에서 생성된 최신 `.log` 파일과 설정 화면/Tab 진단창 캡처를 함께 전달 |
