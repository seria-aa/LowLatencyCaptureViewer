...
examples package v1.2.8 using the `build\Release` output above; the build-directory
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\package-v1.ps1 `
  -Version 1.2.8 -BuildDir ..\build\Release -OutputDir ..\outputs\v1.2.8
```

The outputs are `..\outputs\v1.2.8\LowLatencyCaptureViewer_v1.2.8_Setup.exe`
and `..\outputs\v1.2.8\LowLatencyCaptureViewer_v1.2.8_x64.zip`. When preparing
