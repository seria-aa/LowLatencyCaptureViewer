...
아래 실행 명령은 모두 **v1.2.7**의 실제 예시이며 빌드 폴더는 현재 관례인
`build-v1270-release`를 사용합니다. 다른 버전은 버전·빌드 폴더·출력 경로를
함께 변경합니다. 빌드 폴더에 점이 있는 버전을 넣고 패키지 기본 경로는 그대로
두지 않습니다.

## 1. 버전 표기

Git 태그와 앱 버전 표시에는 `v1.2.7`을 사용합니다. 실행 파일 리소스와 설치
...
파일은 패키지에 넣지 않습니다. ZIP 안의 EXE와 설치 파일 버전이 `1.2.7` 또는
`1.2.7.0`인지 확인합니다. 파일명만 새 버전이고 내부 EXE는 구버전이면 안 됩니다.
...
`--prerelease`를 추가합니다. **v1.2.7은 정식 릴리스**이므로 해당 옵션을 쓰지 않습니다.
