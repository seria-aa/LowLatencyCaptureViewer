...
All executable examples below are for **v1.2.9**, using the compact build-folder
name `build-v1290-release`. For another version, update the version, build
...
Use `v1.2.9` for the Git tag and app version label. Resource and installer
...
```powershell
chcp.com 65001 > $null
cmake -S . -B build-v1290-release -G Ninja -DCMAKE_BUILD_TYPE=Release
cmake --build build-v1290-release
ctest --test-dir build-v1290-release --output-on-failure
git diff --check
```
...
```text
LowLatencyCaptureViewer_v1.2.9_Setup.exe
LowLatencyCaptureViewer_v1.2.9_x64.zip
```

```powershell
chcp.com 65001 > $null
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\package-v1.ps1 `
  -Version 1.2.9 -BuildDir ..\build-v1290-release -OutputDir ..\outputs\v1.2.9
& "C:\Program Files\Inno Setup 7\ISCC.exe" `
  "--define=BuildDir=..\build-v1290-release" ".\installer\LowLatencyCaptureViewer.iss"
```
...
the installer version before uploading: v1.2.9 uses a numeric file version of
`1.2.9` or `1.2.9.0`, not the previous release's executable under a new ZIP name.
...
$releaseArchive = [IO.Compression.ZipFile]::OpenRead(
  (Resolve-Path '..\outputs\v1.2.9\LowLatencyCaptureViewer_v1.2.9_x64.zip').Path)
try {
...
git commit -m "Prepare v1.2.9 release"
git push origin agent/release-v1.0.0
```

Publish from the branch HEAD using the matching release-notes file. Add
`--prerelease` only when a beta is intended; **v1.2.9 is a regular release**.

```powershell
gh release create v1.2.9 `
  "..\outputs\v1.2.9\LowLatencyCaptureViewer_v1.2.9_Setup.exe" `
  "..\outputs\v1.2.9\LowLatencyCaptureViewer_v1.2.9_x64.zip" `
  --repo seria-aa/LowLatencyCaptureViewer `
  --target agent/release-v1.0.0 `
  --title "Low Latency Capture Viewer v1.2.9" `
  --notes-file ".\docs\release-notes-v1.2.9.md"
```
