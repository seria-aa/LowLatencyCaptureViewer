...
All executable examples below are for **v1.2.12**, using the compact build-folder
name `build-v12120-release`. For another version, update the version, build
...
Use `v1.2.12` for the Git tag and app version label. Resource and installer
...
the installer version before uploading: v1.2.12 uses a numeric file version of
`1.2.12` or `1.2.12.0`, not the previous release's executable under a new ZIP name.
...
`--prerelease` only when a beta is intended; **v1.2.12 is a regular release**.
