...
All executable examples below are for **v1.2.6**, using the compact build-folder
name `build-v1260-release`. For another version, update the version, build
...
Use `v1.2.6` for the Git tag and app version label. Resource and installer
...
```powershell
chcp.com 65001 > $null
cmake -S . -B build-v1260-release -G Ninja -DCMAKE_BUILD_TYPE=Release
cmake --build build-v1260-release
ctest --test-dir build-v1260-release --output-on-failure
git diff --check
```
...
```text
LowLatencyCaptureViewer_v1.2.6_Setup.exe
LowLatencyCaptureViewer_v1.2.6_x64.zip
```

```powershell
chcp.com 65001 > $null
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\package-v1.ps1 `
  -Version 1.2.6 -BuildDir ..\build-v1260-release -OutputDir ..\outputs\v1.2.6
& "C:\Program Files\Inno Setup 7\ISCC.exe" `
  "--define=BuildDir=..\build-v1260-release" ".\installer\LowLatencyCaptureViewer.iss"
```
...
the installer version before uploading: v1.2.6 uses a numeric file version of
`1.2.6` or `1.2.6.0`, not the previous release's executable under a new ZIP name.
...
$releaseArchive = [IO.Compression.ZipFile]::OpenRead(
  (Resolve-Path '..\outputs\v1.2.6\LowLatencyCaptureViewer_v1.2.6_x64.zip').Path)
try {
...
git commit -m "Prepare v1.2.6 release"
git push origin agent/release-v1.0.0
```

Publish from the branch HEAD using the matching release-notes file. Add
`--prerelease` only when a beta is intended; **v1.2.6 is a regular release**.

```powershell
gh release create v1.2.6 `
  "..\outputs\v1.2.6\LowLatencyCaptureViewer_v1.2.6_Setup.exe" `
  "..\outputs\v1.2.6\LowLatencyCaptureViewer_v1.2.6_x64.zip" `
  --repo seria-aa/LowLatencyCaptureViewer `
  --target agent/release-v1.0.0 `
  --title "Low Latency Capture Viewer v1.2.6" `
  --notes-file ".\docs\release-notes-v1.2.6.md"
```
