...
All executable examples below are for **v1.2.11**, using the compact build-folder
name `build-v12110-release`. For another version, update the version, build
...
Use `v1.2.11` for the Git tag and app version label. Resource and installer
...
the installer version before uploading: v1.2.11 uses a numeric file version of
`1.2.11` or `1.2.11.0`, not the previous release's executable under a new ZIP name.
...
`--prerelease` only when a beta is intended; **v1.2.11 is a regular release**.
